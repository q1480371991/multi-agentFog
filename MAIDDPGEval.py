...
metrics_path = os.path.join(output_dir, "train/evaluation_metrics.npy")
summary_path = os.path.join(output_dir, "train/evaluation_summary.txt")
...
# 开始评估
env = FogToFogEnv()
# 记录评估指标的列表  每回合奖励、每回合耗时（或延迟）、每回合能量消耗
...
np.save(os.path.join(output_dir+"train/", "evaluation_rewards.npy"), np.array(evaluation_rewards))
np.save(os.path.join(output_dir+"train/", "evaluation_latencies.npy"), np.array(evaluation_latencies))
np.save(os.path.join(output_dir+"train/", "evaluation_energy.npy"), np.array(evaluation_energy))
...
plt.tight_layout()
plt.savefig(os.path.join(output_dir+"train/", "evaluation_plots.png"))
plt.show()
